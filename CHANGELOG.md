...
- Docs: new Decisions page on **Jev**, the optional decision model (`ai.decisionModel`): the 70% confidence rule, every built-in decision point and what Explorbot does when the answer is rejected, and the `judge` tool. Setup is in the AI providers guide.
