...
function abortAfterIdle(ms: number, state: { cancelled: boolean; timedOut: boolean }, controller: AbortController): Promise<never> {
...
export class Provider {
  ...
  private async raceWithIdleTimeout<T>(fn: (signal: AbortSignal) => Promise<T>, timeoutMs: number): Promise<T> {
    ...
    try {
      return await Promise.race([fn(combinedSignal), abortAfterIdle(timeoutMs, state, controller)]);
    } catch (error) {
