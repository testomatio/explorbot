...
import { parseResearchSections, withBlockquoteEntry } from './parser.ts';
...
export function WithLocators<T extends Constructor>(Base: T) {
  return class extends Base {
    ...
    private updateSectionContainer(result: ResearchResult, section: ReturnType<typeof parseResearchSections>[0], newCss: string | null): void {
      ...
      if (newCss) {
        result.text = sectionQuery.query('blockquote[0]').replace(withBlockquoteEntry(section.rawMarkdown, 'Container', `'${newCss}'`));
      } else {
