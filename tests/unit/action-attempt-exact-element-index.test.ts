...
describe('Action.attemptExactElementIndex', () => {
  it('sets elementIndex on the step the command starts, and only on that one', async () => {
    ...
    expect(await Action.prototype.attemptExactElementIndex.call(fake, 'I.click("Pin")', 2)).toBe(true);
  ...
  it('detaches when the command fails before any step starts', async () => {
    ...
    await Action.prototype.attemptExactElementIndex.call(fake, 'I.click("Pin")', 2);
    const step: any = { opts: {} };
